...
export const parsedSetSchema = z
  .object({
    ...
    has_drop_set: z.boolean().default(false),
    drop_set_descriptor: z
      .object({
        drops: z.number().int().positive(),
        reduce_pct: z.array(z.number().nonnegative()).optional(),
...
export const parsedExerciseSchema = z.object({
  ...
  chained_into_next: z.boolean().default(false),
  rest_after_seconds_min: z.number().int().nonnegative().nullable(),
  rest_after_seconds_max: z.number().int().nonnegative().nullable(),
  rest_after_text: z.string().nullable(),
...
export const parsedGroupSchema = z
  .object({
    ...
    short_on_time_remove: z.boolean().default(false),
  ...
  .superRefine((g, ctx) => {
    ...
    if (g.prescription_mode === 'continuous_effort' && !g.progression_text) {
      ctx.addIssue({
        code: 'custom',
        message: 'continuous_effort requires progression_text',
        path: ['progression_text'],
      });
    }
    if (
