...
export const weightRefAbsoluteSchema = z.object({
  ...
  load_kg_male: z.number().positive().nullable(),
  load_kg_female: z.number().positive().nullable(),
...
export const weightRefDeltaFromSetSchema = z.object({
  ...
  delta_percent_max: z.number().nullable(),
});
...
// `z.union` (anyOf) rather than `z.discriminatedUnion` (oneOf) — OpenAI
// Structured Outputs rejects oneOf inside response_format. The `kind` literals
// on each branch still give Zod enough to route parses unambiguously.
export const weightRefSchema = z.union([
