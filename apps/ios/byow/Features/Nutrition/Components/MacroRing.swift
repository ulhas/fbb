...
#Preview {
    HStack(spacing: 24) {
        ZStack {
            MacroRing(progress: 0.65, lineWidth: 14, tint: .byowOrange)
                .frame(width: 100, height: 100)
            VStack {
                Text("65%").font(.byow.metric)
            }
        }
        ZStack {
            MacroRing(progress: 1.18, lineWidth: 14, tint: .byowOrange)
                .frame(width: 100, height: 100)
            VStack { Text("118%").font(.byow.metric) }
        }
    }
    .padding()
