...
/// the four-question quiz from BYOW live (Equipment → Goal → Preference →
        ...
        ToolbarItem(placement: .topBarTrailing) {
            Button(action: onDone) {
                Image(systemName: "xmark")
                    ...
                    .overlay(
                        Circle().strokeBorder(Color.byowDivider.opacity(0.5), lineWidth: 1)
                    )
            ...
            Button(action: stepForward) {
                ...
                .shadow(
                    color: answers.isComplete(through: step)
                        ? Color.byowOrange.opacity(0.35) : .clear,
                    radius: 14, x: 0, y: 6
                )
            }
    ...
    var body: some View {
        HStack(spacing: 6) {
            ForEach(questionSteps, id: \.self) { step in
                Capsule()
                    .fill(state(for: step) == .completed ? Color.byowOrange
                          : state(for: step) == .current ? Color.byowOrange.opacity(0.55)
...
private struct OptionCard: View {
    ...
    var body: some View {
        Button(action: {
            ...
            .overlay(border)
            .shadow(
                color: isSelected ? Color.byowOrange.opacity(0.18) : Color.black.opacity(0.04),
    ...
    private var checkmark: some View {
        ZStack {
            Circle()
                .fill(isSelected ? Color.byowOrange : Color.clear)
                .frame(width: 22, height: 22)
            Circle()
                .strokeBorder(
                    isSelected ? Color.byowOrange : Color.inkMuted.opacity(0.45),
                    lineWidth: 1.6
                )
                .frame(width: 22, height: 22)
            if isSelected {
    ...
    private var border: some View {
        RoundedRectangle(cornerRadius: 16, style: .continuous)
            .strokeBorder(
                isSelected ? Color.byowOrange : Color.inkMuted.opacity(0.18),
...
private struct PrimaryRecommendationCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Spacing.sm) {
                ...
                Button(action: onChoose) {
                    ...
                    .shadow(color: Color.byowOrange.opacity(0.35), radius: 14, x: 0, y: 6)
                }
                .buttonStyle(PressedScaleButtonStyle())
                .disabled(isCommitting)
                .padding(.top, 4)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 22)
                .strokeBorder(Color.byowOrange.opacity(0.35), lineWidth: 1.5)
        )
    ...
    private var familyTint: Color {
        ...
        case "pump_lift", "perform":                            return .byowOrange
        case "pump_condition", "minimalist", "hybrid_running":  return .byowTeal
...
private struct AnswersChipRow: View {
    ...
    private func chip(label: String) -> some View {
        Text(label)
            ...
            .overlay(Capsule().strokeBorder(Color.byowOrange.opacity(0.25), lineWidth: 1))
    }
}
