...
struct AdherenceHeatmapCard: View {
    ...
    private func fill(for status: AdherenceCell.Status) -> Color {
        ...
        case .completed: return Color.byowOrange.opacity(0.65)
        case .exceeded:  return Color.byowOrange
        case .rest:      return Color.byowDivider.opacity(0.5)
        case .future:    return Color.byowDivider.opacity(0.18)
        }
    }
        ...
        HStack(spacing: 4) {
            ForEach([0.18, 0.40, 0.65, 1.0], id: \.self) { opacity in
                RoundedRectangle(cornerRadius: 2)
                    .fill(Color.byowOrange.opacity(opacity))
