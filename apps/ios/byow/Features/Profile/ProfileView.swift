...
struct ProfileView: View {
    ...
    private func loadedContent(data: ProfileData) -> some View {
        ...
        SettingsCard(
            ...
            notificationToggle(symbol: "bell.fill",     symbolTint: .byowOrange, title: "Workout reminders",       keyPath: \.workoutReminders, prefs: data.notifications)
            RowDivider()
            notificationToggle(symbol: "sparkles",      symbolTint: .byowTeal,   title: "Weekly coach insights",   keyPath: \.weeklyInsights,   prefs: data.notifications)
            RowDivider()
            notificationToggle(symbol: "trophy.fill",   symbolTint: .byowOrange, title: "PR celebrations",         keyPath: \.prCelebrations,   prefs: data.notifications)
            RowDivider()
            notificationToggle(symbol: "envelope.fill", symbolTint: .byowTeal,   title: "Coach messages",          keyPath: \.coachMessages,    prefs: data.notifications)
            RowDivider()
        ...
        SettingsCard(
            ...
            privacyToggle(symbol: "eye.fill",            symbolTint: .byowTeal,     title: "Visible to your coach",   subtitle: "Coach Sarah", keyPath: \.shareWithCoach,  prefs: data.privacy)
            RowDivider()
            privacyToggle(symbol: "heart.text.square.fill", symbolTint: .byowTeal,  title: "Sync with Apple Health",  subtitle: "Sleep, HRV, weight, steps", keyPath: \.shareWithHealth, prefs: data.privacy)
            RowDivider()
            PreferenceRow(
            ...
            PreferenceRow(
                ...
                onTap: { info("Account deletion goes through support@byow.training during Phase 1.") }
            )
        }
        ...
        SettingsCard(title: "About") {
            ...
            RowDivider()
            PreferenceRow(symbol: "questionmark.circle.fill", title: "Support",   onTap: { info("Support: support@byow.training") })
        }
                        ...
                        Button { onSelect(p) } label: {
                            HStack(alignment: .top, spacing: Spacing.md) {
                                Image(systemName: initial == p ? "checkmark.circle.fill" : "circle")
                                    ...
                                    .foregroundStyle(initial == p ? Color.byowOrange : Color.inkMuted)
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: Spacing.cardCorner)
                                    .strokeBorder(initial == p ? Color.byowOrange.opacity(0.5) : .clear, lineWidth: 1.4)
                            )
                            .elevation(.card)
                        }
                        .buttonStyle(.plain)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    Button("Save", action: onSave)
                        .font(.byow.bodyBold)
                        .foregroundStyle(canSave ? Color.byowOrange : Color.inkMuted)
                        .disabled(!canSave)
                }
            }
        }
