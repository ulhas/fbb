...
struct TodayNutritionCard: View {
        ...
        HStack(alignment: .top, spacing: Spacing.md) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                MacroLine(
                    ...
                    target: value.target.proteinG, tint: .byowOrange, isPriority: true
                )
                MacroLine(
                    label: "Carbs", logged: value.logged.carbsG,
                    target: value.target.carbsG, tint: .byowTeal
                )
                MacroLine(
    ...
    private func actions(for value: NutritionDay) -> some View {
        HStack(spacing: Spacing.xs) {
            ...
            .background(Color.byowOrange, in: RoundedRectangle(cornerRadius: 10, style: .continuous))
...
private struct MacroLine: View {
    ...
    var body: some View {
        HStack(spacing: Spacing.xs) {
            ...
            Text(label)
                .font(.byow.caption.weight(isPriority ? .bold : .regular))
                .foregroundStyle(Color.inkPrimary)
