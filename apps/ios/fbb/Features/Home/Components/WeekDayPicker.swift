    ...
    private func chevron(systemName: String, enabled: Bool, action: @escaping () -> Void) -> some View {
        Button {
            ...
            Image(systemName: systemName)
                ...
                .foregroundStyle(enabled ? Color.fbbOrange : Color.inkMuted)
                .frame(width: 36, height: 36)
                .background(
                    Color.fbbOrangeTint.opacity(enabled ? 0.45 : 0.18),
                    in: Circle()
                )
        }
        .buttonStyle(.plain)
    ...
    private var background: some View {
        Group {
            if isSelected {
                ...
                Color.fbbBackground.opacity(0.55)
            }
        }
    }
    ...
    private var border: some View {
        if isToday && !isSelected {
            RoundedRectangle(cornerRadius: 12)
                .strokeBorder(Color.fbbOrange.opacity(0.6), lineWidth: 1.4)
        } else if !isSelected {
            RoundedRectangle(cornerRadius: 12)
                .strokeBorder(Color.fbbDivider.opacity(0.35), lineWidth: 1)
        }
    }

    private var letterTint: Color {
        if isSelected { return .white.opacity(0.85) }
        if isToday    { return .fbbOrange }
        return .inkSecondary
    }
