    ...
    var body: some View {
        Group {
            if isLogged {
                ...
                ZStack {
                    Circle().fill(Color.fbbOrangeTint)
                    Circle().strokeBorder(Color.fbbOrange, lineWidth: 3)
                }
            } else {
                Circle()
...
private struct DurationTimerButtonContent: View {
    ...
    private var faceColor: Color {
        if logged || timer?.isCompleted == true { return Color.fbbTeal }
        if timer?.isRunning == true { return Color.fbbOrange.opacity(0.92) }
        return Color.fbbOrange
    }

    private var trackColor: Color {
        if logged || timer?.isCompleted == true { return Color.fbbTeal.opacity(0.4) }
