...
struct PreferenceValue: View {
    ...
    var body: some View {
        Text(text)
            .font(.fbb.caption.weight(.semibold).monospacedDigit())
            .foregroundStyle(Color.inkSecondary)
            .lineLimit(1)
    }
}
...
#Preview {
    VStack(spacing: 0) {
        PreferenceRow(symbol: "envelope.fill", title: "Email", subtitle: "alex@fbb.training") {} trailing: { PreferenceValue(text: "Edit") }
        Divider()
