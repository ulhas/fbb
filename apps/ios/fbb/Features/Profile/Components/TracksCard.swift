    ...
    var body: some View {
        HStack(spacing: Spacing.sm) {
            Image(systemName: familySymbol)
                ...
                .background(Color.fbbOrangeTint.opacity(0.55), in: RoundedRectangle(cornerRadius: 8, style: .continuous))
    ...
    var body: some View {
        Button(action: onToggle) {
            HStack(spacing: Spacing.md) {
                Image(systemName: familySymbol)
                    .font(.system(size: 16, weight: .bold))
                    .foregroundStyle(isSelected ? Color.white : Color.fbbOrange)
                    .frame(width: 40, height: 40)
                    .background(
                        isSelected ? Color.fbbOrange : Color.fbbOrangeTint.opacity(0.55),
                ...
                Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
                    ...
                    .foregroundStyle(isSelected ? Color.fbbOrange : Color.inkMuted)
            }
            .padding(Spacing.md)
            .background(Color.surfaceCard)
            .clipShape(RoundedRectangle(cornerRadius: Spacing.cardCorner, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: Spacing.cardCorner)
                    .strokeBorder(isSelected ? Color.fbbOrange.opacity(0.5) : .clear, lineWidth: 1.4)
            )
            .elevation(.card)
        }
        .buttonStyle(.plain)
