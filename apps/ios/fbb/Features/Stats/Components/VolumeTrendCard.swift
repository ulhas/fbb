...
struct VolumeTrendCard: View {
        ...
        Chart {
            ForEach(deloadDates, id: \.self) { iso in
                if let date = ISODate.parse(iso) {
                    ...
                    .foregroundStyle(Color.fbbTealTint.opacity(0.45))
                }
            }
            ...
            ForEach(points) { p in
                if let date = ISODate.parse(p.weekStartsOn) {
                    ...
                    .foregroundStyle(
                        LinearGradient(
                            colors: [Color.fbbOrange.opacity(0.34), Color.fbbOrange.opacity(0.02)],
                            startPoint: .top,
                            endPoint: .bottom
                        )
                    )
            ...
            if let scrubbed, let date = ISODate.parse(scrubbed.weekStartsOn) {
                ...
                .foregroundStyle(Color.fbbOrangeDark)
                .symbolSize(110)
            }
        }
        .chartYAxis {
    ...
    private var legend: some View {
        HStack(spacing: Spacing.sm) {
            Label {
                Text("Deload").font(.fbb.label).foregroundStyle(Color.inkSecondary)
            } icon: {
                RoundedRectangle(cornerRadius: 2)
                    .fill(Color.fbbTealTint.opacity(0.7))
                    .frame(width: 10, height: 10)
            }
            Label {
                Text("PR").font(.fbb.label).foregroundStyle(Color.inkSecondary)
            } icon: {
