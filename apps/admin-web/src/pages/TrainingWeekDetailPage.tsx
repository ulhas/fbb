...
// Exported so the route file can reuse it in validateSearch.
...
export function TrainingWeekDetailPage() {
  const { weekStartsOn } = useParams({ from: '/training-weeks/$weekStartsOn' })
  const search = useSearch({ from: '/training-weeks/$weekStartsOn' })
  const navigate = useNavigate({ from: '/training-weeks/$weekStartsOn' })
